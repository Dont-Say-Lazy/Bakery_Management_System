...
<h1>Confirm Warehouse Stock Action</h1>
...
<div style="background-color: <%= backgroundColor %>; color: <%= textColor %>; padding: 15px; margin-bottom: 20px; border-radius: 5px; border-left: 5px solid <%= borderColor %>;">
    ...
    <p>You are about to <%= actionVerb %> <strong><%= quantity %></strong> units of <strong><%= fruitName %></strong> <%= actionType.equals("checkIn") ? "to" : "from" %> <strong><%= locationName %></strong> warehouse:</p>
    <ul>
